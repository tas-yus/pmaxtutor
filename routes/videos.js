...
router.post("/videos", middleware.isLoggedIn, middleware.isAdmin, (req, res) => {
    ...
    Part.findOne({code: req.params.partCode}, (err, part) => {
        ...
        file.mv(__dirname + '/public/assets/videos/' + path, (err) => {
            if (err) {
                return console.log(err);
            }
            getVideoInfo('./public/assets/videos/small.mp4').then(info => {
                var newVid = {
        ...
        //                 return console.log(err);
        //             }
        //             part.videos.push(video);
        //             ctr++;
        //             if (ctr === req.body.video.length) {
        //                 part.save((err) => {
        //                     if (err) {
        //                         return console.log(err);
        //                     }
